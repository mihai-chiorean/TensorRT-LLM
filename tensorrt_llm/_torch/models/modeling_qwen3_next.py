...
class Qwen3NextSparseMoeBlock(nn.Module):
    ...
    def __init__(
        ...
        # global quant_algo is MIXED_PRECISION, which does not have has_nvfp4()
        # set.  This causes get_moe_cls() to fall back to CutlassFusedMoE with
        # UnquantizedFusedMoEMethod, which cannot load uint8 NVFP4 weights.
        # Resolve the per-layer expert quant config from quant_config_dict so
        # that get_moe_cls() sees the correct (e.g. NVFP4) algo and dispatches
        # to the right NVFP4CutlassFusedMoEMethod.
        #
        # The quant_config_dict keys use the HF checkpoint module path
        # (model.language_model.layers.N.mlp.experts for VLM-style nesting, or
        # model.layers.N.mlp.experts for plain-text checkpoints), so we try
        # both forms before falling back to the global quant_config.
